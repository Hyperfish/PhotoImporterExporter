...
namespace Hyperfish.ImportExport.O365
{
    public static class ClientContextExtension
    {
        ...
        public static void ExecuteQueryWithIncrementalRetry(this ClientContext context, int retryCount, int delay, ILog logger = null)
        {
            int retryAttempts = 0;
            ...
            while (retryAttempts < retryCount)
            {
                ...
                catch (WebException wex)
                {
                    ...
                    if (response != null && (response.StatusCode == (HttpStatusCode)429 || response.StatusCode == (HttpStatusCode)503))
                    {
                        ...
                        logger?.Debug(string.Format("CSOM request frequency exceeded usage limits. Sleeping for {0} seconds before retrying.", backoffInterval));
